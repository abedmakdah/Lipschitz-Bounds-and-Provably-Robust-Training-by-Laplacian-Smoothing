function [DELTA, Accuracy_nominal, Accuracy_adversarial]  = RL_adversarial_perturbation...
...
for r = 1 : N_adv_test
    ...
    if predicted_class == true_class
        ...
        %             D=vecnorm(X_vertex-closest_vertex)/2; %distance between vertex and closes vertex (might be zero)
...
Accuracy_nominal = length(find(abs(predicted_class_test - true_class_test) <= 10^-4))/size(Y_test,2);

[~, predicted_class_adv] = max(F(:, knnsearch(X_vertex', (X_test+Delta)')), [], 1);

Accuracy_adversarial = length(find(abs(predicted_class_adv - true_class_test) <= 10^-4))/size(Y_test,2);
